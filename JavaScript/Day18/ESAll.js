...
function txAdd() {
  ...
  transactions.push({ id: txId++, name, amount, cat });

  // ES2024: Object.groupBy()
  const grouped = Object.groupBy(transactions, (t) => t.cat);

  // Render groups
  const container = document.getElementById("tx-groups");
  container.innerHTML = Object.entries(grouped)
    .map(
      ([cat, items]) => `
    <div class="group-section">
      <div class="group-label">${cat} (${items.length} giao dịch)</div>
      <div class="chips">
        ${items.map((i) => `<span class="chip">${i.name} — ${i.amount.toLocaleString("vi-VN")}đ</span>`).join("")}
      </div>
    </div>`,
    )
    .join("");
  ...
  const totals = Object.fromEntries(
    Object.entries(grouped).map(([k, v]) => [
      k,
      v.reduce((s, i) => s + i.amount, 0),
    ]),
  );
  out("tx-out", [
    line(`// ES2024: Object.groupBy(transactions, t => t.cat)`, "muted"),
    ...Object.entries(totals).map(
      ([k, v]) =>
        `${line(k, "key")}: ${line(v.toLocaleString("vi-VN") + " đ", "val")}`,
    ),
  ]);
...
function runResolvers() {
  ...
  btn.disabled = true;
  document.getElementById("res-confirm").style.display = "block";
  ...
  out("res-out", [
    line(`// ES2024: Promise.withResolvers()`, "muted"),
    line(`const { promise, resolve, reject } = Promise.withResolvers()`),
    line(`// resolve/reject dùng được ở bất kỳ đâu ngoài executor`),
    line(`⏳ Đang chờ user chọn...`, "val"),
  ]);

  // Auto-reject sau 5s
  setTimeout(() => {
    if (_resolver) {
      _resolver.reject(new Error("Timeout sau 5 giây"));
      _resolver = null;
    }
  }, 5000);
  ...
  promise
    .then((ok) => {
      out("res-out", [
        line(`// resolve(${ok}) được gọi từ bên ngoài executor`, "muted"),
        line(
          ok ? `✓ User xác nhận — đã xoá dữ liệu` : `✗ User huỷ — không làm gì`,
          ok ? "ok" : "val",
        ),
      ]);
    })
    .catch((err) => {
      out("res-out", [line(`✗ ${err.message}`, "err")]);
    })
    .finally(() => {
...
function resolverAnswer(ok) {
  if (_resolver) {
    _resolver.resolve(ok);
    _resolver = null;
  }
}

// ES2025 — SET METHODS
function runSetMethods() {
  const a = new Set(
    document
      .getElementById("set-a")
      .value.split(",")
      .map((s) => s.trim())
      .filter(Boolean),
  );
  ...
  out("set-out", [
    line(`// ES2025: Set Methods`, "muted"),
    `${line("A", "key")}: ${line(JSON.stringify([...a]), "val")}`,
    `${line("B", "key")}: ${line(JSON.stringify([...b]), "val")}`,
    line(`────────────────────────────`, "muted"),
    `${line("A.union(B)", "key")}:              ${line(JSON.stringify([...union]), "ok")}`,
    `${line("A.intersection(B)", "key")}:      ${line(JSON.stringify([...inter]), "ok")}`,
    `${line("A.difference(B)", "key")}:        ${line(JSON.stringify([...diffA]), "ok")}`,
    `${line("B.difference(A)", "key")}:        ${line(JSON.stringify([...diffB]), "ok")}`,
    `${line("A.symmetricDiff(B)", "key")}: ${line(JSON.stringify([...symDiff]), "ok")}`,
    `${line("A.isSubsetOf(B)", "key")}:       ${line(a.isSubsetOf(b), "val")}`,
    `${line("A.isDisjointFrom(B)", "key")}: ${line(a.isDisjointFrom(b), "val")}`,
  ]);
}

// ES2025 — PROMISE.TRY
let ptMode = "sync-ok";
function setPTMode(m, btn) {
  ptMode = m;
  document
    .querySelectorAll("#s-promise-try .tab")
    .forEach((t) => t.classList.remove("active"));
  btn.classList.add("active");
}

function runPromiseTry() {
  const handlers = {
    "sync-ok": () => ({ name: "An", score: 9.0 }),
    "sync-err": () => {
      throw new Error("Sync: dữ liệu không hợp lệ");
    },
    "async-ok": async () => {
      await fakeDelay(600);
      return { name: "Bình", score: 7.5 };
    },
    "async-err": async () => {
      await fakeDelay(600);
      throw new Error("Async: server lỗi");
    },
  };

  const logs = [
    line(`// ES2025: Promise.try() — bọc sync/async đồng nhất`, "muted"),
  ];
  logs.push(line(`mode: "${ptMode}"`, "val"));
  logs.push(line(`⏳ chạy...`));
  out("pt-out", logs);

  // ES2025: Promise.try()
  Promise.try(handlers[ptMode])
    .then((result) => {
      out("pt-out", [
        line(`// ES2025: Promise.try()`, "muted"),
        line(`✓ .then() — kết quả:`, "ok"),
        `${line("name", "key")}:  ${line(`"${result.name}"`, "val")}`,
        `${line("score", "key")}: ${line(result.score, "val")}`,
      ]);
    })
    .catch((err) => {
      out("pt-out", [
        line(`// ES2025: Promise.try()`, "muted"),
        line(`✗ .catch() — dù là sync hay async throw:`, "err"),
        `${line("message", "key")}: ${line(`"${err.message}"`, "err")}`,
      ]);
    });
}
